...
test.skip('Неавторизованный пользователь может получить список статей', async ({ request }) => {
    const response = await request.get('https://realworld.qa.guru/api/articles');
    expect(response.status()).toBe(200);
});

test.skip('Неавторизованный пользователь может получить список статей с пагинацией', async ({ request }) => {
